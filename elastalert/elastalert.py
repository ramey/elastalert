...
class ElastAlerter():
    ...
    def format_buckets(self, bucket):
        return [bucket_name, bucket_query] 
    ...
    def get_buckets_aggregation_query(self, rule):
        ...
        for bucket in rule['buckets']:
            ...
            bucket_queries.append([bucket_obj.name, bucket_obj.bucket_query])
        for i in range(len(bucket_queries) - 1, -1, -1):
            query = bucket_queries[i]
            query[1][query[0]]['aggs'] = bucket_query
            bucket_query = query[1]
        return bucket_query
            
        
    def get_aggregation_query(self, query, rule, timestamp_field='@timestamp'):
        """ Takes a query generated by get_query and outputs a  aggregation query"""
        aggs_query = {}
        query_element = query['query']
        if 'sort' in query_element:
            query_element.pop('sort')
        if rule['buckets']:
            aggs_query['aggs'] = self.get_buckets_aggregation_query(rule)
        else: 
            aggs_query['aggs'] = rule['aggregation_query_element']
        aggs_query['query'] = query_element
        return aggs_query
    ...
    def get_segment_size(self, rule):
        ...
        if not rule.get('use_count_query') and not rule.get('use_terms_query') and not rule.get('aggregation_query_element'):
            ...
            if rule.get('bucket_interval'):
                return rule.get('bucket_interval')
            else:
                return rule.get('buffer_time', self.buffer_time)
    ...
    def run_rule(self, rule, endtime, starttime=None):
        ...
        if len(rule['type'].buckets_to_compare) > 0:
            rule['type'].compare_buckets()
        ...
        while rule['type'].matches:
            ...
            if self.is_silenced(rule['name'] + "._silence") or self.is_silenced(silence_cache_key) and not rule.get('aggregation_query_element'):
...
class Bucket():
    def __init__(self, bucket):
        ...
        bucket_objs = bucket[self.bucket_type][0][self.name]
        bucket_query_obj = {}
        for bucket_obj in bucket_objs:
            for bucket_obj_key, bucket_obj_value  in bucket_obj.iteritems():
                bucket_query_obj[bucket_obj_key] = bucket_obj_value
        self.bucket_query = {self.name: {self.bucket_type: bucket_query_obj}}
    def get_bucket_query(self):
        return self.bucket_query
